...
Deno.test('REST Responder - Collection POST', async () => {
  ...
  assertEquals((store.lastUpdatedState as any[])[0].name, 'New Item')
})
...
Deno.test('REST Responder - Item GET/PUT/PATCH/DELETE', async () => {
  ...
  assertEquals(JSON.parse(resPut?.body || '{}').name, 'Updated 1')
  assertEquals(JSON.parse(resPut?.body || '{}').id, '1')
  assertEquals((store.lastUpdatedState as any[])[0].name, 'Updated 1')
  ...
  assertEquals((store.lastUpdatedState as any[])[1].description, 'New field')
  ...
  assertEquals(store.lastUpdatedState?.length, 1)
  assertEquals((store.lastUpdatedState as any[])[0].id, '2')
})
