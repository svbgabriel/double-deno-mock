...
Deno.test('REST Responder - Item GET/PUT/PATCH/DELETE', async () => {
  ...
  // PUT
  const resPut = await engine.handleRequest(
  ...
  // PATCH
  const resPatch = await engine.handleRequest(
    new Request('http://localhost/items/2', {
      method: 'PATCH',
      headers: { 'content-type': 'application/json' },
  ...
  assertExists(store.lastUpdatedState)
  assertArrayIncludes(store.lastUpdatedState, [{
    id: patchBody.id,
  ...
  // DELETE
  const resDelete = await engine.handleRequest(
    new Request('http://localhost/items/1', {
      method: 'DELETE',
    }),
  )
